...
class TestExecutionTools:
    ...
    async def delete_test_execution(self, issue_id: str) -> Dict[str, Any]:
        ...
        if "data" in result and "deleteTestExecution" in result["data"]:
            return {
                "success": result["data"]["deleteTestExecution"],
                "issueId": issue_id,
            }
        else:
    ...
    async def add_tests_to_execution(
        self, execution_issue_id: str, test_issue_ids: List[str]
    ) -> Dict[str, Any]:
    ...
    async def remove_tests_from_execution(
        ...
        if "data" in result and "removeTestsFromTestExecution" in result["data"]:
            return {
                "success": result["data"]["removeTestsFromTestExecution"],
                "executionId": execution_issue_id,
            }
        else:
            raise GraphQLError(
                f"Failed to remove tests from execution {execution_issue_id}"
            )

    async def add_test_environments(
        self, execution_issue_id: str, test_environments: List[str]
    ) -> Dict[str, Any]:
        ...
        if "data" in result and "addTestEnvironmentsToTestExecution" in result["data"]:
            ...
            raise GraphQLError(
                f"Failed to add test environments to execution {execution_issue_id}"
            )

    async def remove_test_environments(
        self, execution_issue_id: str, test_environments: List[str]
    ) -> Dict[str, Any]:
        ...
        if (
            "data" in result
            and "removeTestEnvironmentsFromTestExecution" in result["data"]
        ):
            return {
                "success": result["data"]["removeTestEnvironmentsFromTestExecution"],
                "executionId": execution_issue_id,
            }
        else:
            raise GraphQLError(
                f"Failed to remove test environments from execution {execution_issue_id}"
            )
