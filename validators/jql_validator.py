...
class JQLValidator:
    ...
    ALLOWED_FIELDS: Set[str] = {
        ...
        "reporter", "created", "updated", "resolved", "summary",
        "description", "labels", "components", "fixVersion",
        "affectedVersion", "environment", "resolution", "key",
        
        # Xray-specific fields
        "testType", "testPlan", "testExecution", "testEnvironment",
        "requirement", "testSet", "defect", "testRun", "testCycle",
    ...
    ALLOWED_KEYWORDS: Set[str] = {
        ...
        "in", "is", "was",  # Common JQL keywords
        "d", "w", "m", "y", "h"  # Time duration suffixes (days, weeks, months, years, hours)
    }
    
    # Whitelisted JQL functions
    ALLOWED_FUNCTIONS: Set[str] = {
        "currentUser", "currentLogin", "membersOf",
    ...
    def validate_and_sanitize(self, jql: str) -> str:
        ...
        if max_depth > self.MAX_NESTING_DEPTH:
            raise ValidationError(f"JQL nesting too deep (max {self.MAX_NESTING_DEPTH} levels)")
    ...
    def _validate_fields(self, jql: str) -> None:
        ...
        for field in fields:
            ...
            if (field_lower in self.ALLOWED_KEYWORDS or 
                field_lower in self.ALLOWED_FUNCTIONS):
                continue
            ...
            if field_pos != -1 and field_pos + len(field) < len(jql_without_parens):
                ...
                # Skip whitespace
                while next_char_pos < len(jql_without_parens) and jql_without_parens[next_char_pos].isspace():
                    next_char_pos += 1
                if next_char_pos < len(jql_without_parens) and jql_without_parens[next_char_pos] == '(':
            ...
            if field.startswith('cf') and '[' in field and ']' in field:
                ...
                if cf_match:
                    ...
                    if 10000 <= field_num <= 20000:  # Reasonable range for custom fields
                        continue
    ...
    def _validate_operators(self, jql: str) -> None:
        ...
        sql_keywords = ["select", "from", "where", "join", "union", "insert", "update", "delete"]
