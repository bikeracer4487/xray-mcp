...
async def demonstrate_manual_test_creation():
    ...
    try:
        ...
        test_summary = (
            f"Login Flow Test - Demo - {datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
        test_description = (
            "Automated demonstration of Manual test creation with structured steps"
        )
        ...
        steps_match = True
        for i, (created_step, retrieved_step) in enumerate(
            zip(test_data["steps"], retrieved_test["steps"])
        ):
            if (
                created_step["action"] != retrieved_step["action"]
                or created_step["result"] != retrieved_step["result"]
            ):
                steps_match = False
                break
