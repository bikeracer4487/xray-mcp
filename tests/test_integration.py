...
class TestServerIntegration:
    ...
    async def test_server_initialization(self):
        ...
        with patch.dict('os.environ', {
            ...
            for tool_name in expected_tools:
                assert any(tool_name in name for name in tool_names), \
    ...
    async def test_end_to_end_test_creation_and_execution(self):
        ...
        mock_client.execute_mutation = AsyncMock(return_value={
            "data": {
                "createTest": {
                    "test": {
                        "issueId": "TEST-301",
...
class TestAbstractionLayerIntegration:
    ...
    async def test_repository_pattern_integration(self):
        ...
        result = await repo.fetch_data(
            "query { getTest(issueId: $id) { issueId } }",
            {"id": "TEST-123"}
        )
