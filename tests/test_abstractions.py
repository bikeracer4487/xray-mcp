...
try:
    from abstractions import (
        BaseTool, Repository, GraphQLRepository, CachedRepository,
        tool_error_handler, validate_required, ToolFactory
    )
...
class MockRepository(Repository):
    ...
    def set_response(self, query_prefix: str, variables: Dict[str, Any], response: Dict[str, Any]):
...
class SampleTool(BaseTool):
    ...
    async def validated_method(self, required_param: str, optional_param: str = None) -> Dict[str, Any]:
...
class TestDecorators:
    ...
    async def test_validate_required(self):
        ...
        result = await tool.validated_method(required_param="test", optional_param="optional")
        assert result["required"] == "test"
        assert result["optional"] == "optional"
        
        # Invalid call missing required parameter
        with pytest.raises(ValidationError, match="Missing required parameters: required_param"):
