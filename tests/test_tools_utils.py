...
class TestUtilityTools:
    ...
    async def test_execute_jql_query_for_tests(self, utility_tools, mock_client):
        ...
        with patch('tools.utils.validate_jql') as mock_validate:
            ...
            mock_validate.assert_called_once_with('project = "TEST" AND status = "Done"')
    ...
    @pytest.mark.asyncio
    async def test_execute_jql_query_for_test_executions(self, utility_tools, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_execute_jql_query_case_insensitive_entity_type(self, utility_tools, mock_client):
    ...
    async def test_execute_jql_query_validation_error(self, utility_tools):
        ...
        with patch('tools.utils.validate_jql') as mock_validate:
            mock_validate.side_effect = ValidationError("JQL contains dangerous patterns")
            
            with pytest.raises(ValidationError) as exc_info:
                await utility_tools.execute_jql_query(
                    jql="project = TEST; DROP TABLE users;",
                    entity_type="test"
                )
    ...
    async def test_validate_connection_no_data(self, utility_tools, mock_client):
        ...
        mock_client.execute_query.return_value = {"errors": [{"message": "Unauthorized"}]}
    ...
    async def test_execute_test_jql_error_handling(self, utility_tools, mock_client):
        ...
        mock_client.execute_query.return_value = {"errors": [{"message": "Query failed"}]}
    ...
    @pytest.mark.asyncio
    async def test_execute_test_execution_jql_error_handling(self, utility_tools, mock_client):
        ...
        with patch('tools.utils.validate_jql') as mock_validate:
            ...
            assert "Failed to execute JQL query for test executions" in str(exc_info.value)