...
class XrayAuthManager:
    ...
    def __init__(self, client_id: str, client_secret: str, base_url: str = "https://xray.cloud.getxray.app"):
                ...
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        try:
                            ...
                            if "exp" in decoded:
                                self.token_expiry = datetime.fromtimestamp(decoded["exp"], tz=timezone.utc)
                    ...
                    # Handle specific error responses with meaningful messages
                        ...
                        raise AuthenticationError("Unauthorized: Invalid Xray license or credentials")
                    elif response.status == 500:
                        raise AuthenticationError("Internal server error during authentication")
                    else:
                        # Catch-all for unexpected status codes
                        error_text = await response.text()
                        raise AuthenticationError(f"Authentication failed with status {response.status}: {error_text}")
